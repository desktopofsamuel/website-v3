...
export default function EngagementGallery({
  engagements,
}: EngagementGalleryProps) {
  const items: LightboxItem[] = engagements.map((e) => ({
    src: e.image,
    alt: e.title,
    title: e.title,
    subtitle: e.subtitle,
  ...
  return (
    <Gallery items={items}>
      <EngagementRows engagements={engagements} />
    </Gallery>
  );
}

function EngagementRows({ engagements }: { engagements: Engagement[] }) {
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-6 items-start">
            <button
              ...
              onClick={(e) =>
                open(i, e.currentTarget.getBoundingClientRect())
              }
